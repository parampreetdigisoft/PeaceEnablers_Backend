...
{    
    public class PdfGeneratorService : IPdfGeneratorService
    {
        ...
        public async Task<byte[]> GenerateCityDetailsPdf(AiCitySummeryDto cityDetails, List<AiCityPillarResponse> pillars, List<KpiChartItem> kpis, UserRole userRole)
        {
            try
            {
        ...
        public void AddCityDetailsPdf(IDocumentContainer container, AiCitySummeryDto cityDetails, List<AiCityPillarResponse> pillars, List<KpiChartItem> kpis, UserRole userRole, bool isAllCities = false)
        {
            // KPI chart items capped at 107
            var kpiChartItems = kpis.Take(107).ToList();            
            var pillarChartItems = pillars            
            ...
            foreach (var p in accessiblePillars.Where(x=>x.AIProgress.HasValue))
            {
                container.Page(page =>
                {
                    ApplyPageDefaults(page);
        ...
        static void PaintSpiderChart(SKCanvas canvas, Size size, List<KpiChartItem> pillars)
         {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                canvas.DrawText(
                    Shorten(pillars[i].ShortName ?? pillars[i].Name, 6),
            ...
            container.Column(column =>
            {
                ...
                column.Item().Background("#336b58").Padding(12).Column(col =>
                {
                    ...
                    col.Item().PaddingTop(3)
                        .Text($"{data.CityName}, {data.State}, {data.Country} | Data Year: {data.Year}")
    ...
    IContainer container, AiCitySummeryDto data, UserRole userRole)
        {
            ...
            {              
                ...
                column.Item().PaddingTop(10)
                    .Element(c => PillarProgressSection(c, random, userRole));
    ...
    IContainer container, AiCityPillarResponse data, UserRole userRole)
        {
            container.PaddingTop(8).Column(column =>
            {
                ...
                column.Item().PageBreak();
                column.Item().PaddingTop(10).Element(c =>
                    PillarContentSection(c, "Red Flags", data.RedFlag, "#6e9688"));
