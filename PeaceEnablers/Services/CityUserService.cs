...
namespace PeaceEnablers.Services
{
    public class CityUserService : ICityUserService
    {
        ...
        public async Task<Tuple<string, byte[]>> ExportCompareCities(CompareCityRequestDto c, int userId, string tierName)
        {
            try
            {
                var result = await CompareCities(c, userId, tierName, false);
